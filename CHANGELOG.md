...
### 2026-08-27 - Moderator Booking Management UI (#99325, #99326, #99327)

**Người thực hiện:** Trần Trung Hiếu

**Added**

- Tạo trang danh sách Booking dành cho `MODERATOR` và `ADMIN` tại `GET /moderator/bookings`.
- Hiển thị thông tin Booking gồm người đặt, Venue, Space, thời gian bắt đầu/kết thúc, tổng tiền, trạng thái và thời gian tạo.
- Bổ sung bộ lọc theo `venueId`, khoảng thời gian (`fromDate`, `toDate`) và `status`, kèm phân trang.
- Bổ sung `venueId` vào `BookingSearchRequest` và hỗ trợ lọc Booking theo quan hệ `Booking -> Space -> Venue`.
- Bổ sung `venueId`, `venueName` vào `BookingResponse` để phục vụ giao diện danh sách.
- Tạo modal xem chi tiết Booking (#99327), mở từ nút **Xem chi tiết** trên từng dòng.
- Tích hợp API `GET /api/moderator/bookings/{bookingId}` để lấy dữ liệu chi tiết Booking.
- Hiển thị trong modal: người đặt, Venue, Space, thời gian Booking, tổng tiền, trạng thái và phương thức thanh toán.
- Bổ sung `paymentMethod` vào `BookingResponse` và lấy thông tin Payment theo `bookingId`.
...
### 2026-08-26 - Tạo Modal/Drawer xem chi tiết User (#99301)

**Người thực hiện:** Trần Trung Hiếu

#### Added

- Thêm Drawer xem chi tiết người dùng từ màn hình quản lý User.

- Hiển thị thông tin cá nhân: ID, họ tên, email, số điện thoại, ngôn ngữ, vai trò và trạng thái tài khoản.

- Hiển thị ảnh CCCD và giấy phép kinh doanh.

- Thêm trạng thái xác minh danh tính và xác minh doanh nghiệp.

- Thêm action duyệt KYC danh tính.

- Thêm action duyệt giấy phép kinh doanh.

- Thêm action khóa/mở khóa tài khoản.

- Hỗ trợ đóng Drawer bằng nút đóng, overlay và phím Escape.

#### Changed

- Chuyển action trong Drawer sang gọi REST API bằng `fetch()` với Bearer JWT.

- Bổ sung cập nhật URL browser bằng `window.history.replaceState()`.

- Giữ lại keyword, status và role filter khi reload danh sách User sau action.

- Giữ nguyên Search/Filter/Pagination của task `#99297`.

- Giữ nguyên Moderator Layout của task `#99293`.

#### API Used

- `PUT /api/moderator/users/{id}/status`

- `PUT /api/moderator/users/{id}/verify-identity`

- `PUT /api/moderator/users/{id}/verify-business`
...
- Test hiển thị CCCD/Giấy phép kinh doanh.

- Test khóa User: `ACTIVE -> BLOCKED`.

- Test mở khóa User: `BLOCKED -> ACTIVE`.

- Test duyệt KYC danh tính.

- Test duyệt giấy phép kinh doanh.

- Regression test Search/Filter/Pagination.

- Regression test Moderator Layout/Navigation.
